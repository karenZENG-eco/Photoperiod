...
First, to prepare the tree for analysis by setting a minimum branch length of 0.001 so the tree is dichotomous and doesn't have multichotomies. We will also repeat analysis where instead of forcing a positive value to zero branch lengths we will drop them entirely (tree2) in case it makes a difference.
