...
uspn <- uspn %>% 
 ...
 unite(uspn$species, uspn$Genus, uspn$Species)
  
uspn <- uspn %>%
    transmute(species = paste(Genus, species), #combine genus species
            year = str_sub(Observation_Date, start = -4), #extract year by taking last 4 characters of the date
            doy = Day_of_Year, 
            location = State, 
            lat = Latitude,
            lon = Longitude,
...
#pep725.list %>%
#  map(add_filename, input_folder = "./data/raw/EU_PEP725/PEP725_FT/", output_folder = "./data/output/") #apply function to everything

pep725 <- list.files("./data/output", full.names = T) %>% map_df(read.csv)#read in the whole folder

pep725 <- pep725 %>% filter(BBCH == "60") #filter for first flowering (BBCH = 60)

pep725$location <- str_sub(pep725$filename, start = 8, end = 9) #country code
pep725$species <- str_sub(pep725$filename, start = 11, end = -5) #species name
pep725$source <- "pep725"


#add country based on filename
#add source
