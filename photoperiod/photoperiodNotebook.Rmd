...
write.csv(data, "./graphdata.csv") #save as csv
...
- Anova between two linear models significant
- Bootstrapping also reveals significant difference
...
Photo.FFD.Comparison <- getpval(data)
```


##
- We can also look at the phenological advancement of each species individually.
...
- However, species data is likely skewed from the different sources that are seperate populations with vastly different climates (especially when comparing between a regular population and a rocky mountain population)...
